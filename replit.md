...
- **Comment User Attribution**: Fixed comment attribution system to properly identify actual users for both general comments and meeting comments. Modified POST /api/comments and POST /api/meetings/:meetingId/comments endpoints in server/routes.ts to read X-User-Email header (already sent by queryClient.ts) and identify the real user via getUserByEmail(), then use that user's ID as authorId. This ensures comments display the correct author name instead of defaulting to test users, matching the pattern previously implemented for activity feed attribution.
- **Meeting Comment Edit/Delete Permissions**: Implemented author-only permission controls for meeting comments. Added inline edit functionality with textarea UI and save/cancel buttons. Enhanced backend security by validating permissions via X-User-Email header instead of trusting client-provided requesterId. Added updateMeetingComment methods to both MemStorage and DrizzleStorage implementations.
- **Meeting Comment Edit Indicators**: Added "(수정됨)" indicator to edited comments, displayed when updatedAt differs from createdAt. Comment timestamps always show original creation time (createdAt), ensuring comment ordering remains stable after edits. Modified updatedAt is only used for displaying the edit indicator.
- **Meeting Comment Delete Confirmation**: Wrapped delete button in AlertDialog component requiring user confirmation before deletion. Dialog displays "댓글 삭제" title with "이 댓글을 삭제하시겠습니까? 이 작업은 되돌릴 수 없습니다." message and "취소"/"삭제" buttons, preventing accidental comment deletions.