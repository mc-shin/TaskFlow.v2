...
export const projects = pgTable("projects", {
  ...
  labels: text("labels")
    .array()
    .default(sql`'{}'`), // Labels (최대 2개)
  ownerIds: text("owner_ids")
    .array()
    .default(sql`'{}'`), // Multiple owners
...
export const goals = pgTable("goals", {
  ...
  assigneeIds: text("assignee_ids")
    .array()
    .default(sql`'{}'`), // Multiple assignees
...
export const insertProjectWithValidationSchema = insertProjectSchema.refine(
  (data) => {
...
export const insertGoalWithValidationSchema = insertGoalSchema.refine(
  (data) => {
    return !data.labels || data.labels.length <= 2;
  },
  {
...
export const insertTaskWithValidationSchema = insertTaskSchema.refine(
  (data) => {
    return !data.labels || data.labels.length <= 2;
  },
  {
...
export const insertMeetingCommentSchema = createInsertSchema(
  meetingComments
).omit({
  id: true,
  createdAt: true,
