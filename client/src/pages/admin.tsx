                      ...
                      <div className="space-y-2">
                        <div className="flex gap-2">
                          <div className="flex-1">
                            <Input
                              ...
                              onChange={(e) => {
                                ...
                                if (inviteForm.formState.errors.email) {
                                  inviteForm.clearErrors("email");
                                }
                              }}
                              onBlur={() => {
                                // Validate on blur to show errors immediately
                                const email = inviteForm.getValues("email");
                                if (!email.trim()) {
                                  ...
                                  // Trigger validation for format
                                  inviteForm.trigger("email");
                                }
                              }}
                              className={`bg-slate-700 border-slate-600 text-white placeholder:text-slate-400 ${
                                inviteForm.formState.errors.email ? 'border-red-500' : ''
                              }`}
                              data-testid="input-invite-email"
                            />
                            {inviteForm.formState.errors.email && (
                              <p className="text-red-400 text-sm mt-1" data-testid="text-email-error">
                                {inviteForm.formState.errors.email.message}
                              </p>
                            )}
                          </div>
                          <Select value={inviteForm.watch("role")} onValueChange={(value) => inviteForm.setValue("role", value as "관리자" | "팀원")}>
                          ...
                          <Button 
                            ...
                            onClick={(e) => {
                              e.preventDefault();
                              // Manually validate if email is empty to show error
                              const email = inviteForm.getValues("email");
                              if (!email.trim()) {
                                inviteForm.setError("email", {
                                  type: "required",
                              ...
                              inviteForm.handleSubmit(handleInviteSubmit)();
                            }}
